import skincareImage from "@/assets/skincare-routine.jpg";

const steps = [
  { step: "1", name: "Oil Cleanser", purpose: "Remove makeup & sunscreen" },
  { step: "2", name: "Water Cleanser", purpose: "Deep clean pores" },
  { step: "3", name: "Exfoliator", purpose: "2-3x weekly for smooth skin" },
  { step: "4", name: "Toner", purpose: "Balance & prep skin" },
  { step: "5", name: "Essence", purpose: "Boost hydration" },
  { step: "6", name: "Treatment", purpose: "Target specific concerns" },
  { step: "7", name: "Sheet Mask", purpose: "Intensive care 2-3x weekly" },
  { step: "8", name: "Eye Cream", purpose: "Delicate eye area care" },
  { step: "9", name: "Moisturizer", purpose: "Lock in hydration" },
  { step: "10", name: "Sunscreen", purpose: "SPF 50+ daily (AM only)" },
];

const SkincareRoutine = () => {
  return (
    <section id="skincare" className="py-20 bg-background">
      <div className="container mx-auto px-4">
        <div className="text-center mb-16">
          <h2 className="font-display text-4xl md:text-5xl font-bold text-foreground mb-4">
            The 10-Step K-Beauty Routine
          </h2>
          <p className="text-lg text-muted-foreground max-w-2xl mx-auto">
            The secret to glass skin isn't just products—it's the method. Here's how to layer like a pro.
          </p>
        </div>

        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center max-w-6xl mx-auto">
          <div className="order-2 lg:order-1">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {steps.map((item) => (
                <div
                  key={item.step}
                  className="bg-card border border-border rounded-xl p-4 hover:border-primary/50 transition-all duration-300 hover:shadow-lg"
                >
                  <div className="flex items-start gap-3">
                    <div className="w-8 h-8 rounded-full bg-primary/10 text-primary flex items-center justify-center font-bold text-sm flex-shrink-0">
                      {item.step}
                    </div>
                    <div className="flex-1 min-w-0">
                      <h3 className="font-semibold text-foreground mb-1 text-sm">
                        {item.name}
                      </h3>
                      <p className="text-xs text-muted-foreground">
                        {item.purpose}
                      </p>
                    </div>
                  </div>
                </div>
              ))}
            </div>
            <div className="mt-6 bg-secondary/10 border border-secondary/20 rounded-xl p-4">
              <p className="text-sm text-foreground">
                <span className="font-semibold">Remember:</span> You don't need all 10 steps every day! Start with the basics (cleanser, toner, moisturizer, SPF) and add products as needed.
              </p>
            </div>
          </div>

          <div className="order-1 lg:order-2">
            <div className="rounded-2xl overflow-hidden shadow-2xl">
              <img
                src={skincareImage}
                alt="Korean skincare routine products"
                className="w-full h-full object-cover"
              />
            </div>
          </div>
        </div>
      </div>
    </section>
  );
};

export default SkincareRoutine;
